 ...
 * @author Daniel
 */
public final class CanonBallModel implements IExplodingProjectile {
    ...
    public void update(float tpf) {
        if (exploding) {
            explodingTimer += tpf;
            if (explodingTimer > CanonBallModel.EXPLOSION_END_TIME) {
                pcs.firePropertyChange(EXPLOSION_FINISHED, null, null);
            }
        } else {
            projectileLifeTimer += tpf;
            if (projectileLifeTimer > CanonBallModel.MAX_LIFE_TIME) {
